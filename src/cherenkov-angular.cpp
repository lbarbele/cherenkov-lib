	...
	const ParticleType primaryParticle)
{
  ...
  auto parameters = Parameters(showerAge, refractiveIndex, showerEnergyTeV, primaryParticle);

  // Compute the maximum Cherenkov emission angle
  double thetaEm = std::acos(1.0/refractiveIndex);

  return UnnormalizedDensity(theta, thetaEm, parameters)
    / UnnormalizedIntegral(0.0, Constants::MaxTheta, thetaEm, parameters);
}
	...
	const ParticleType primaryParticle)
{
  ...
  return UnnormalizedIntegral(0.0, theta, thetaEm, parameters)
    / UnnormalizedIntegral(0.0, Constants::MaxTheta, thetaEm, parameters);
}
...
namespace {
		...
		const std::array<double,4> parameters)
	{
		...
		if (theta < thetaEm - delta) {
		  ...
		  return UnnormalizedIntegral(thetaEm - delta, thetaEm + delta, thetaEm, parameters) / (2*delta);
		} else {
